...
const STORAGE_KEY = 'journey_architect_progress_v1';
const XP_PER_LEVEL = 100;

const DEFAULT_PROGRESS = {
    teamName: 'Architect Team',
    xp: 0,
    currentLevel: 1,
    unlockedLevels: [1]
};
...
const JourneyArchitectContext = createContext(null);

export function JourneyArchitectProvider({ children }) {
    ...
    return <JourneyArchitectContext.Provider value={value}>{children}</JourneyArchitectContext.Provider>;
}

export function useJourneyArchitect() {
    const ctx = useContext(JourneyArchitectContext);
    if (!ctx) {
        throw new Error('useJourneyArchitect must be used inside JourneyArchitectProvider');
    }
    return ctx;
}
